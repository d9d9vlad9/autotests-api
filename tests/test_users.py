...
@pytest.mark.parametrize("email", [fake.email(domain="mail.ru"), fake.email(domain="gmail.com"), fake.email(domain="example.com")], ids=lambda email: f"email: {email}")
...
@pytest.mark.users
@pytest.mark.regression
def test_get_me(private_users_client: PrivateUsersClient, function_user):
